...
async def read_news(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_async_session)):
...
async def search_news_by_tag(tag: str, skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_async_session)):
...
# async def create_news(news: schemas.NewsCreate, db: AsyncSession = Depends(get_async_session), user = Depends(get_current_active_admin)):
...
# async def update_news(news_id: int, news: schemas.NewsUpdate, db: AsyncSession = Depends(get_async_session), user = Depends(get_current_active_admin)):
#     result = await db.execute(select(models.News).filter(models.News.id == news_id).options(joinedload(models.News.tags)))
#     db_news = result.scalar_one_or_none()
#     if db_news is None:
#         raise HTTPException(status_code=404, detail="News not found")
...
# async def delete_news(news_id: int, db: AsyncSession = Depends(get_async_session), user = Depends(get_current_active_admin)):
